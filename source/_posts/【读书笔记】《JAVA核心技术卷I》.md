...
如果将一个类声明为 final，只有其中的方法自动成为 final，而**不包括字段**。
...
可以通过调用 `getState` 方法来查看线程当前的状态。

![](/Users/asssaver/github/blog/source/images/20230117/4.jpeg)

#### 新建线程

> p556

当用 `new` 操作符创建一个新线程时，如 `new Thread(r)`，这个线程还没开始运行，此时状态是*新建*（new）。

#### 可运行线程

> p556

一旦调用 `start` 方法，线程就处于*可运行*（runnable）状态。

抢占式调度系统给每一个可运行线程一个时间片来执行任务，当时间片用完时，操作系统**剥夺**该线程的运行权，并给另一个线程一个机会来运行。

现在**所有**的桌面以及服务器操作系统都使用抢占式调度。但是像手机这样的小型设备可能使用协作式调度：一个线程只有在调用 `yield` 方法或者被阻塞或等待时才失去控制权。

#### 阻塞和等待线程

> p556

当线程处于阻塞或等待状态时，它暂时是不活动的，要由线程调度器重新激活这个线程。

* 当一个线程试图获取一个内部的对象锁，而这个锁目前被其他线程占有，该线程会被**阻塞**。
* 当线程等待另一个线程通知调度器出现一个条件是，这个线程会进入**等待**状态。
* 有几个方法有超时参数，调用这些方法会让线程进入***计时等待***（timed waiting）状态。
...
多个线程之间可能会互相争取并覆盖对同一个数据的存取，这种情况被称为*竞态条件*（race condition）。
...
**一定**要把 `unlock` 操作放在 `finally` 子句中，确保即使抛出异常了也能释放锁。

**NOTE：**使用锁时，就**不能**使用 try-with-resources 语句。

这种锁称为*重入*（reentrant）锁，因为线程可以反复获得已拥有的锁。锁有一个*持有计数器*（hold count）来跟踪对 `lock` 方法的嵌套调用。例如，在方法 A 中调用了方法 B，那么此时 `ReentrantLock` 对象的持有计数为 2，当方法 B 退出时，持有计数变为1，当方法 A 退出时，持有计数变为 0，此时线程释放锁。

#### 条件对象

> p571

可以使用一个*条件对象*（也叫*条件变量*（conditional variable））来管理那些已经获得了一个锁却因为不满足其他条件而不能往下执行的线程。一个锁对象可以有一个或多个相关联的条件对象，可以用 `newCondition` 方法来获得一个条件对象。例如：

```java
Condition conditionMet = myLock.newCondition();
```

当发现条件不满足时，调用 `await()` 方法，通常应该写在等待条件满足的循环中。例如：

```java
myLock.lock();
try
{
  while ( conditions not being met )
  {
    conditionMet.await();
  }
  . . .
...
一旦一个线程调用了 `await` 方法，它就进入这个条件的*等待集*（wait set）。当锁可用时，该线程**并不会**变为可运行状态，它仍保持非活动状态，直到**其他线程**在同一条件对象上调用 `signalAll` 方法。例如：

```java
conditionMet.signalAll();
```

这个调用会重新激活等待这个条件的所有线程，被激活的线程进入可运行状态，并**从之前暂停的地方继续执行**。被激活的线程应该再次测试条件，因为 `signalAll` 方法仅仅代表现在有可能满足条件。

#### synchronized 关键字

> p576

大多数情况下不需要使用上述那么复杂的锁。Java 中的**每个对象**都有一个内部锁，而用 `synchronized` 关键字声明的方法会被该内部锁保护。也就是说，
...
内部对象锁**有且仅有一个**关联条件对象，`wait` 方法将一个线程增加到等待集中，`notifyAll` 或 `notify` 方法可以解除等待线程的阻塞。

#### 同步块

> p579

还有一种机制可以获得锁：即进入一个*同步块*。例如：
...
它会截获 obj 对象的锁为己用。使用一个对象的锁来实现额外的原子操作的这种做法称为*客户端锁定*（clientside locking）。但是客户端锁定是非常脆弱的，通常不推荐使用。

#### 监视器概念

> p580

*监视器*（monitor）是不要求程序员考虑显式锁就可以保证多线程安全性的一种方案，用 Java 的术语讲，监视器具有如下特性：
...
如果写一个变量，而这个变量接下来可能会被另一个线程读取；或者如果读一个变量，而这个变量可能已经被另一个线程修改，那么必须使用同步。
...
如果将一个字段声明为 `final`，那么其他线程会在构造器完成构造以后才能看到该变量。
